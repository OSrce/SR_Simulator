...
sub new
{
	...
	$dbh = DBI->connect("DBI:Pg:dbname=sitrepdev;host=localhost", "sitrepadmin", "", {'RaiseError' => 1});
	
	bless $self, $class;
	switch($self->{_category}) {
			case "Fire"			{ 
				#generate random data
				$self->{_type} = $self->generateType();
				$self->{_location} = generateAddress();
				$self->{_structure} = generateStructure();				
					}
			case "Police"		{ 
				#generate random data
				$self->{_type} = $self->generateType();
    ...
    print "New CFS #$self->{_num}: $self->{_type}\n";
...
sub generateType {
	...
	switch($self->{_category}) {
			case "Fire"			{ 
				@typelist = ("Fire alarm", "Fire", "Smoke alarm", "Electrical fire", "Chemical fire", "Smoke", "Structure fire", "Cat stuck in a tree");
					}
			case "Police"		{ 

				@typelist = ("Burglary", "Larceny", "Grand Theft Auto", "Assault", "Sexual Assault", "Homicide");
							
					}
			case "Medical"		{ 
				@typelist = ("Shortness of breath", "Choking victim", "Heart attack", "Car accident", "Victim unconscious", "Gunshot wound", "Broken leg");
	...
	return $typelist[int(rand(@typelist))];
}

sub generateStructure{

		my @structurelist = ("Condo", "Highrise", "Skyscraper", "Warehouse", "House", "Garage");

	return $structurelist[int(rand(@structurelist))];
}	
...
sub generateUpdates {
	...
	switch($self->{_category}) {
			case "Fire"			{ 
					my ( $self ) = @_;
					#return ("New CFS #$self->{_num}: $self->{_type} at $self->{_location} . . .", "Engine assigned . . . ", "$self->{_structure} . . .", "Engine en route . . .", "Additional caller ...", "Engine on scene . . .", "Situation resolved . . .");
					return ("New CFS #$self->{_num}: $self->{_type} . . . ", "$self->{_location} . . .", "$self->{_structure} . . .", "Address verified . . .");
			...
			case "Police"		{ 
					...
					return ("New CFS #$self->{_num}: $self->{_type} at $self->{_location} . . .", "$perpdesc . . . ", "$self->{_vehicle} . . .", "$vicdesc . . . ");
								
					}
			case "Medical"		{ 
					my ( $self ) = @_;
					my $vicdesc = $self->getVictimDescription();
					#return ("New CFS #$self->{_num}: $self->{_type} at $self->{_location} . . .", "Ambulance assigned . . . ", "Ambulance en route . . . ", "$vicdesc . . . ", "Ambulance on scene . . .", "Victim in stable condition . . .", "Situation resolved . . .");
					return ("New CFS #$self->{_num}: $self->{_type} . . ." , "$self->{_location} . . .", "$vicdesc . . . ", "Additional caller . . .");
					}
...
sub getUpdate {
	...
	#Maybe the eventdemo should call update on every cycle, and this function should decide whether or not there's anything new to update (i.e. rand()<0.1)
	
#	if($self->{updatenum} == 5){
#		my $entityid="";
		#Check to see if an engine is on scene yete
#		$query = "select es.entity from event ev, entity_status es where ev.id=$eventid AND es.has_end='f' AND es.data @> '\"assigned\"=>\"f\"' AND es.data @> '\"inservice\"=>\"t\"'";								  
...
#		$query_handle->fetch();	
		...
		#TODO If it's not onscene yet, how do I make sure the eventdemo doesn't increase the update number. Maybe this should keep track of the update number instead....
#		if ($entityid !=""){
...
#	}			
	...
	#Check if unit is assign
	
	#Check if unit is onscene
	
	if($self->{updatenum} - 1  >= @tmp){
		my $tmpnum = $self->{updatenum} -1;
		#return "Update $tmpnum . . .";
		#If the number of updates is greater than the number we have available to give, then don't give anything
		return "";
	} else {
		return $tmp[$self->{updatenum}-1];
	}
